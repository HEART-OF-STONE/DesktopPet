...
import { compactNumber, quotaDuration, quotaWindows, quotaName, quotaObservation, quotaFreshnessReason, timeLabel, estimateLabel } from './core/integrations';
import { desktopAction } from './platform/bridge';
import { summaryEstimate, summaryQuotas } from './core/petSummary';
...
export function PetAgentStatus({agent,quiet,scale=1}:{agent:AgentPresentation;quiet:boolean;scale?:number}){
  ...
  return <div style={{zoom:scale}} className={`pet-agent-status ${card?'pet-side-panel':''} ${bottom?'pet-bottom-panel':''} status-${status.kind}`}>
    ...
    {open&&createPortal(<section className={`pet-agent-card ${bottom?'pet-detail-bottom':''}`} aria-label="桌宠用量详情">
      ...
      {data.quota.windows.length?quotaWindows(data.quota).map(w=><div key={`${w.bucket}:${w.label}`}><div className="pet-data-row"><span>{quotaName(w)} · {quotaDuration(w.windowMinutes)}</span><strong>剩余 {(100-w.usedPercent).toFixed(0)}%</strong></div><p className="pet-data-note">{quotaObservation(w,clock)}{quotaFreshnessReason(w,clock,data.settings.codexEnabled)&&` · ${quotaFreshnessReason(w,clock,data.settings.codexEnabled)}`}</p></div>):<div className="pet-data-row"><span>Codex 额度</span><span>暂无数据</span></div>}
