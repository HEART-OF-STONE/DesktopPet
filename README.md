...
**当前交付：v0.1.0 可运行原型。** 首版方向已确定为静态 PNG + 程序变形、逐帧 2D 两种表现；AI 和 Live2D 留在后续阶段。公开发布前仍需完成实际鼠标穿透、多屏与长时间运行验收。

![管理界面](output/playwright/home.png)

## 直接体验

双击本次构建的 [DesktopPet.exe](output/DesktopPet/DesktopPet.exe)。它是 Windows x64 可执行文件，界面和内置素材已打包，需要本机有 WebView2 运行时。当前提供免安装原型，尚未制作和验证安装包。
