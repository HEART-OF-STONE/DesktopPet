...
**源码版本：v0.8.0（开发中，尚未发布正式 Release）。** 在偏好设置选择数据框自动 / 固定左侧 / 固定右侧，单独调整 80%–125% 大小。所在屏幕的前台应用全屏时临时隐藏，退出后恢复；手动隐藏状态仍保留。保留三种数据框样式、低频自动额度查询、悬停控制条、提醒及备份恢复。内置 AI 聊天、Live2D 与自动安装更新留在后续阶段。

![改名界面](docs/images/rename-dialog.png)

## 直接体验

通过 [GitHub Releases](https://github.com/HEART-OF-STONE/DesktopPet/releases) 分发验收后的安装包、免安装 ZIP 和 SHA-256 清单。当前尚未发布正式版本，请先按下方命令从源码构建；本机 `output/` 中的测试产物不作为公开下载。面向 Windows x64；安装包仅安装到当前用户，需要 WebView2，缺少时安装程序会尝试联网安装运行时。签名、干净系统、多屏和长期运行验收仍在跟进。详见 [运行、演示与分享说明](docs/08-sharing.md)。
...
- **今日装扮**：内置奶油白、鼠尾草两套配色。
- **偏好设置**：宠物大小、置顶、吸附、免打扰、音量、移回主屏、完整备份恢复、开机启动与手动更新检查。
